...
Rails.application.configure do
  ...
  config.after_initialize do
    if AnyCable::Rails.enabled?
      config.action_cable.url = ActionCable.server.config.url = ENV.fetch("CABLE_URL", "ws://34.125.184.224:3334/cable")
    end
  end
  ...
  config.hosts << "34.125.184.224"
  config.hosts << "website" if ENV['EXERCISM_DOCKER']
  config.hosts << /.*.ngrok.io/
end

Rails.application.routes.default_url_options = {
  host: "http://34.125.184.224:3020"
}
